 ...
 * 下拉列表
 */
public class FilterDropdownListView extends ScrollView {

    private Context mContext;

    public LinearLayout linearLayout;

    public FilterDropdownItemObject current;

    List<? extends FilterDropdownItemObject> list;

    public FilterDropdownButton button;
    ...
    public FilterDropdownListView(Context context) {
        this(context, null);
    }

    public FilterDropdownListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FilterDropdownListView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void flush() {
        for (int i = 0, n = linearLayout.getChildCount(); i < n; i++) {
            ...
            if (view instanceof FilterDropdownListItemView) {
                FilterDropdownListItemView itemView = (FilterDropdownListItemView) view;
                setDropdownListItemViewStyle(itemView);
                FilterDropdownItemObject data = (FilterDropdownItemObject) itemView.getTag();
                if (data == null) {
                    return;
                }
    ...
    public void bind(List<? extends FilterDropdownItemObject> list,
        ...
        LinkedList<FilterDropdownListItemView> cachedViews = new LinkedList<>();
        for (int i = 0, n = linearLayout.getChildCount(); i < n; i++) {
            View view = linearLayout.getChildAt(i);
            if (view instanceof FilterDropdownListItemView) {
                cachedViews.add((FilterDropdownListItemView) view);
            } else {
...
//        LayoutInflater inflater = LayoutInflater.from(getContext());
        ...
        for (FilterDropdownItemObject item : list) {
            if (isFirst) {
                isFirst = false;
            } else {
            ...
            FilterDropdownListItemView view = cachedViews.poll();
            if (view == null) {
            ...
            view.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    FilterDropdownItemObject data = (FilterDropdownItemObject) v.getTag();
                    if (data == null) return;
                    FilterDropdownItemObject oldOne = current;
                    current = data;
                    flush();
                    container.hide();
                    if (oldOne != current) {
                        container.onSelectionChanged(FilterDropdownListView.this);
                    }
                }
            });
        ...
        button.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (getVisibility() == VISIBLE) {
                    ...
                    container.show(FilterDropdownListView.this);
                }
            }
        });
     ...
     */
    @NonNull
    private FilterDropdownListItemView genDropdownListItemView() {
        FilterDropdownListItemView view;
        view = new FilterDropdownListItemView(mContext);
    ...
    public static interface Container {
        void show(FilterDropdownListView listView);

        void hide();

        void onSelectionChanged(FilterDropdownListView view);
    }
    ...
    private void setDropdownListItemViewStyle(FilterDropdownListItemView itemView) {
        if (itemTextSize != -1)
